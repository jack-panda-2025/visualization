"""
export_vtp_v2.py
导出带车壳的 vtp 文件，护栏用 solid 单元（有应力），车辆用 shell 单元（无应力）
"""
...
print(f"Solid 护栏: {solid_is_barrier.sum():,}, 车辆: {(~solid_is_barrier).sum():,}")
print(f"Shell 车辆: {shell_is_vehicle.sum():,}")
del d3_geom
...
for i in range(len(timesteps)):
    ...
    # 护栏网格（solid hex8）
    barrier_conn_i = solid_conn[solid_is_barrier]
    n_b = len(barrier_conn_i)
    cell_arr = np.hstack([np.full((n_b,1),8,dtype=np.int64),
                          barrier_conn_i.astype(np.int64)]).ravel()
    cell_types = np.full(n_b, 12, dtype=np.uint8)
    barrier_mesh = pv.UnstructuredGrid(cell_arr, cell_types, coords_def.astype(np.float64))
    ...
    # 车辆网格（shell quad4）
    vehicle_conn_i = shell_conn[shell_is_vehicle]
    n_v = len(vehicle_conn_i)
    cell_arr_v = np.hstack([np.full((n_v,1),4,dtype=np.int64),
                             vehicle_conn_i.astype(np.int64)]).ravel()
    cell_types_v = np.full(n_v, 9, dtype=np.uint8)
    vehicle_mesh = pv.UnstructuredGrid(cell_arr_v, cell_types_v, coords_def.astype(np.float64))
